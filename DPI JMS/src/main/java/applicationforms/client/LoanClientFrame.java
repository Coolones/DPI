...
public class LoanClientFrame extends JFrame {
	...
	public LoanClientFrame() {
		...
		btnQueue.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				LoanRequest request = new LoanRequest(listModel.getSize() + 1, ssn,amount,time);
   ...
   public RequestReply<LoanRequest,LoanReply> getRequestReply(LoanRequest request){
    ...
    public RequestReply<LoanRequest,LoanReply> getRequestReply(LoanReply reply) {

        for (int i = 0; i < listModel.getSize(); i++){
            RequestReply<LoanRequest,LoanReply> rr =listModel.get(i);
            if (rr.getRequest().getId() == reply.getId()){
                return rr;
            }
        }
   ...
   public void SetReply(LoanReply reply) {
       for (int i = 0; i < listModel.getSize(); i++){
           RequestReply<LoanRequest,LoanReply> rr =listModel.get(i);
           if (rr.getRequest().getId() == reply.getId()){
               rr.setReply(reply);
